...
router.get("/stats/summary", async (_req, res): Promise<void> => {
  const [totals] = await db.select({ totalLinks: count(trackingLinksTable.id) }).from(trackingLinksTable);
  const [logTotals] = await db.select({ totalVisits: count(ipLogsTable.id), uniqueIps: countDistinct(ipLogsTable.ip) }).from(ipLogsTable);
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const [todayVisits] = await db.select({ visitsToday: count(ipLogsTable.id) }).from(ipLogsTable).where(sql`${ipLogsTable.createdAt} >= ${today.toISOString()}`);
  const topLinks = await db.select().from(trackingLinksTable).orderBy(desc(trackingLinksTable.visitCount)).limit(5);
  const recentLogs = await db
    .select({
      id: ipLogsTable.id, linkId: ipLogsTable.linkId, linkName: trackingLinksTable.name,
      ip: ipLogsTable.ip, userAgent: ipLogsTable.userAgent, country: ipLogsTable.country,
      region: ipLogsTable.region, city: ipLogsTable.city, zip: ipLogsTable.zip,
      lat: ipLogsTable.lat, lon: ipLogsTable.lon, gpsAccuracy: ipLogsTable.gpsAccuracy,
      timezone: ipLogsTable.timezone, isp: ipLogsTable.isp, org: ipLogsTable.org,
      asn: ipLogsTable.asn, mobile: ipLogsTable.mobile, proxy: ipLogsTable.proxy,
      hosting: ipLogsTable.hosting, referrer: ipLogsTable.referrer, photo: ipLogsTable.photo,
      createdAt: ipLogsTable.createdAt,
    })
    .from(ipLogsTable)
    .leftJoin(trackingLinksTable, eq(ipLogsTable.linkId, trackingLinksTable.id))
    .orderBy(desc(ipLogsTable.createdAt)).limit(10);
  ...
  res.json({
    ...
    recentLogs: recentLogs.map((l) => serializeLog({ ...l, linkName: l.linkName ?? null })),
  });
});
